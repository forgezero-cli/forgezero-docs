...
const contentsCandidates = [
  join(docsPath, "CONTENTS.md"),
  join(docsPath, "CONTENTS"),
];
...
function detectAlertVariant(text) {
  ...
  if (lower.startsWith("**conclusion:**") || lower.startsWith("conclusion:"))
    return "info";
  return "note";
}
...
function stripAlertPrefix(text) {
  return text
    .replace(/^\*\*(Note|Warning|Tip|Conclusion):\*\*\s*/i, "")
    .replace(/^(Note|Warning|Tip|Conclusion):\s*/i, "");
}
  ...
  for (const token of tokens) {
    switch (token.type) {
      ...
      case "table":
        blocks.push({
          ...
          rows: token.rows.map((row) =>
            row.map((cell) => inlineTokensToHtml(cell.tokens)),
          ),
        });
        break;
      case "list":
        blocks.push({
          ...
          items: token.items.map((item) => ({
            ...
            nested: item.tokens?.slice(1)
              ? tokenToBlocks(item.tokens.slice(1))
              : [],
          })),
        });
        break;
      case "hr":
        ...
        break;
      case "html":
        if (token.block) blocks.push({ type: "html", content: token.text });
        break;
      case "space":
        break;
      default:
...
function buildNavigation(sections) {
  ...
  return groupRanges
    .map((group) => ({
      ...
      items: [
        ...(group.from === 0
          ? [sections.find((s) => s.slug === "introduction")].filter(Boolean)
          : []),
        ...numbered
...
function readMarkdownDocsIndex() {
  ...
  for (const line of lines) {
    ...
    if (/^READING ORDER/i.test(trimmed)) break;

    const linkMatch = trimmed.match(/\[[^\]]+\]\(([^)]+)\)/);
    const candidate = linkMatch
      ? linkMatch[1]
      : trimmed.replace(/^[-*]\s*/, "").replace(/^\d+\.\s*/, "");
    const cleaned = candidate.replace(/^\./, "").replace(/^\//, "").trim();

    if (!cleaned || /^https?:/i.test(cleaned) || cleaned === "...") continue;
...
function extractSectionsFromDocs() {
  ...
  for (let i = 0; i < docOrder.length; i++) {
    ...
    const number =
      numberByDocName.get(entry) ?? numberByDocName.get(name) ?? i + 1;
    const slug = uniqueSlug(baseSlug || slugify(name), number);
    const blocks = parseSectionContent(md);

    sections.push({ id: slug, number, title, slug, blocks });
  }
...
function buildContent() {
  ...
  if (
    !navigation.some((g) => g.items?.some?.((it) => it.slug === "introduction"))
  ) {
    const intro = sections.find((s) => s.slug === "introduction");
    if (intro) {
      const gettingStarted = navigation.find(
        (g) => g.label === "Getting Started",
      );
      if (
        gettingStarted &&
        !gettingStarted.items.some((it) => it.slug === "introduction")
      ) {
        gettingStarted.items.unshift({
          title: intro.title,
          slug: intro.slug,
          id: intro.id,
        });
...
export { buildContent, readMarkdownDocsIndex };

function main() {
  const output = buildContent();
  mkdirSync(dirname(outputPath), { recursive: true });
  writeFileSync(outputPath, JSON.stringify(output, null, 2));
  console.log(`Generated ${output.sections.length} sections to ${outputPath}`);
}
